...
export const CompanyProfilePanel: React.FC = () => {
  ...
  const handleSaveCompany = async () => {
    ...
    setBusy(true);
    try {
      const saved = await updateCompany(companyForm);
      if (!saved) return;
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="text-[10px] rounded-full border-primary/30 text-primary bg-primary/5 font-bold"
              >
                موثقة نظامياً
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5 font-medium">
              البيانات الرسمية المعتمدة للمنشأة، السجلات التجارية، الحسابات البنكية، والربط الحكومي
              السعودي (قوى، مقيم، مدد، التأمينات)
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={busy}
            className="rounded-full text-xs font-bold gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90 h-9 px-4"
          >
        ...
        {[
          {
            ...
            value: money(totals?.balance ?? 1250000),
            icon: Wallet,
            color: "text-primary",
          },
          {
            label: "إجمالي الرواتب المصروفة (WPS)",
            value: money(totals?.paidOut ?? 845000),
            icon: Landmark,
            color: "text-emerald-600",
          },
          {
            label: "سلف وقروض قيد التحصيل",
            value: money(totals?.loansOutstanding ?? 65000),
            icon: ShieldCheck,
          ...
          <div
            key={card.label}
            className="rounded-3xl border border-border/80 bg-card p-4 shadow-xs flex items-center justify-between"
          >
            <div>
            ...
            <div
              className={`h-10 w-10 rounded-2xl bg-secondary flex items-center justify-center ${card.color}`}
            >
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="rounded-3xl border border-border/80 bg-card p-5 shadow-xs space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {renderField(
              "الاسم النظامي (إنجليزي)",
              companyForm.legalNameEn,
              (v) => setCompanyForm((c) => ({ ...c, legalNameEn: v })),
              { dir: "ltr" },
            )}
            {renderField(
              "رقم السجل التجاري (CR) *",
              companyForm.crNumber,
              (v) => setCompanyForm((c) => ({ ...c, crNumber: v })),
              { placeholder: "1010XXXXXX", dir: "ltr" },
            )}
            {renderField(
              "الرقم الضريبي (VAT) *",
              companyForm.taxNumber,
              (v) => setCompanyForm((c) => ({ ...c, taxNumber: v })),
              { placeholder: "3XXXXXXXXXXXX03", dir: "ltr" },
            )}
            {renderField(
              "الرقم الموحد (700)",
              companyForm.unifiedNumber,
              (v) => setCompanyForm((c) => ({ ...c, unifiedNumber: v })),
              { placeholder: "700XXXXXXX", dir: "ltr" },
            )}
            {renderField(
              "رقم المنشأة بالتأمينات (GOSI)",
              companyForm.gosiNumber,
              (v) => setCompanyForm((c) => ({ ...c, gosiNumber: v })),
              { placeholder: "اشتراك التأمينات", dir: "ltr" },
            )}
            {renderField(
              "رقم المنشأة بمكتب العمل / قوى",
              companyForm.laborOfficeNumber,
              (v) => setCompanyForm((c) => ({ ...c, laborOfficeNumber: v })),
              { placeholder: "ملف مكتب العمل", dir: "ltr" },
            )}
            {renderField(
              "النشاط الاقتصادي الرئيسي",
              companyForm.industry,
              (v) => setCompanyForm((c) => ({ ...c, industry: v })),
        ...
        <div className="rounded-3xl border border-border/80 bg-card p-5 shadow-xs space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {renderField(
              "المدينة",
              companyForm.city,
              (v) => setCompanyForm((c) => ({ ...c, city: v })),
              { placeholder: "الرياض" },
            )}
            {renderField(
              "الدولة",
              companyForm.country,
              (v) => setCompanyForm((c) => ({ ...c, country: v })),
              { placeholder: "المملكة العربية السعودية" },
            )}
            <div className="sm:col-span-2">
              {renderField(
                "المقر الرئيسي / العنوان الوطني (واصل)",
                companyForm.headquartersAddress,
                (v) => setCompanyForm((c) => ({ ...c, headquartersAddress: v })),
          ...
          <div className="rounded-3xl border border-border/80 bg-card p-5 shadow-xs space-y-3.5">
            ...
            <div className="space-y-2.5">
              {renderField(
                ...
                (v) => setAccountForm((a) => ({ ...a, bankName: v })),
                { placeholder: "مصرف الراجحي / البنك الأهلي" },
              )}
              {renderField(
                "اسم الحساب *",
                accountForm.accountName,
                (v) => setAccountForm((a) => ({ ...a, accountName: v })),
                { placeholder: "حساب الرواتب والعمليات" },
              )}
              {renderField(
                "رقم الآيبان (IBAN) *",
                accountForm.iban,
                (v) => setAccountForm((a) => ({ ...a, iban: v.toUpperCase() })),
              ...
              <div className="grid grid-cols-2 gap-2">
                {renderField(
                  ...
                  (v) => setAccountForm((a) => ({ ...a, currency: v })),
                  { dir: "ltr" },
                )}
                {renderField(
                  "الرصيد الافتتاحي",
                  String(accountForm.currentBalance),
                  (v) => setAccountForm((a) => ({ ...a, currentBalance: Number(v) || 0 })),
                  { type: "number", dir: "ltr" },
                )}
              </div>
            ...
            <div className="flex gap-2 pt-2">
              <Button
                ...
                disabled={busy}
                className="rounded-full text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90 h-9 px-4 flex-1"
              >
                <Save className="h-3.5 w-3.5" />
          ...
          <div className="lg:col-span-2 rounded-3xl border border-border/80 bg-card p-5 shadow-xs space-y-3.5">
            <div className="flex items-center justify-between border-b border-border/60 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-black text-foreground">
                  قائمة الحسابات البنكية المعتمدة
                </h4>
              </div>
                ...
                <tbody className="divide-y divide-border/50">
                  {accounts.map((account) => (
                    <tr key={account.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td
                        className="px-3 py-3 font-mono font-bold text-[11px] text-foreground ltr:text-left"
                        dir="ltr"
                      >
                        {account.iban}
                      </td>
                      ...
                      <td className="px-3 py-3">
                        {account.isPrimary ? (
                          ...
                          <Badge
                            variant="outline"
                            className="text-[10px] rounded-full font-medium text-muted-foreground"
                          >
                            حساب تشغيلي فرعي
                          </Badge>
                        )}
                  ...
                  {accounts.length === 0 && (
                    <tr>
                      ...
                      >
                        لا توجد حسابات بنكية مضافة حالياً. أضف الحساب البنكي الرئيسي أعلاه لربطه
                        بمسير الرواتب وحماية الأجور.
                      </td>
                    </tr>
                  )}
                </tbody>
