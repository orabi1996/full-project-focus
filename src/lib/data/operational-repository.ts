...
export async function fetchOperationalSnapshot(
  ...
  return {
    ...
    settlements: (settlementsResult.data ?? []).map((row) => ({
      ...
      noticePeriodServed: row.notice_period_served ?? false,
      serviceYears: row.service_years,
      serviceMonths: row.service_months,
      eosbAmount: numberValue(row.eosb_amount),
      leaveBalancePayoutDays: numberValue(row.leave_payout_days),
      leaveBalancePayoutAmount: numberValue(row.leave_payout_amount),
      pendingSalaryAmount: numberValue(row.pending_salary_amount),
      loanDeductionAmount: numberValue(row.loan_deduction_amount),
      assetClearanceComplete: row.asset_clearance_complete ?? false,
      netSettlementAmount: numberValue(row.net_settlement_amount),
      eosbNotes: row.eosb_notes ?? undefined,
...
export async function createSettlementRecord(settlement: Omit<FinalSettlementRecord, "id">) {
  const { error } = await enterpriseSupabase.from("settlements").insert({
    ...
    notice_period_served: settlement.noticePeriodServed,
    leave_payout_days: settlement.leaveBalancePayoutDays,
    leave_payout_amount: settlement.leaveBalancePayoutAmount,
    pending_salary_amount: settlement.pendingSalaryAmount,
    loan_deduction_amount: settlement.loanDeductionAmount,
    asset_clearance_complete: settlement.assetClearanceComplete,
    eosb_notes: settlement.eosbNotes ?? null,
