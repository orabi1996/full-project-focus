...
export const IntegrationsView: React.FC = () => {
  ...
  const erpPlatforms = [
    { id: 'odoo', name: 'Odoo ERP v17', status: 'connected', desc: 'مزامنة قيود الرواتب والمصروفات ومراكز التكلفة' },
    ...
    { id: 'oracle', name: 'Oracle Fusion Cloud', status: 'ready', desc: 'تصدير قيود الأستاذ العام GL' },
  ];
  ...
  const handleSyncJournals = () => {
    alert('تم إرسال قيود الرواتب والمصروفات إلى نظام Odoo بنجاح! كود الاستجابة: 200 OK (Batch ID: JRN-9823)');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b pb-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => setIsSyncJournalOpen(true)} size="sm" className="font-bold text-xs gap-1.5 bg-primary">
            <RotateCw className="h-4 w-4" />
            ترحيل القيود إلى ERP الآن
          </Button>
          <Button onClick={() => setIsAddWebhookOpen(true)} variant="outline" size="sm" className="font-bold text-xs gap-1.5">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 text-xs py-2 font-mono">
            <div className="rounded-lg border bg-muted/20 p-3 space-y-1">
              ...
              <p className="text-muted-foreground">المدين (Debit): 448,500 ر.س (مصروفات رواتب وبدلات)</p>
              <p className="text-muted-foreground">الدائن (Credit): 448,500 ر.س (مستحقات بنك + تأمينات + سلف)</p>
              <span className="text-emerald-600 font-bold block pt-1">✓ القيد متوازن ومطابق (Balanced)</span>
            </div>
          </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 text-xs py-2">
            ...
            <div className="space-y-1">
              ...
              >
                <option value="payroll.confirmed">اعتماد وقفل مسير الرواتب (payroll.confirmed)</option>
          ...
          <DialogFooter className="mt-2">
            <Button size="sm" onClick={handleCreateWebhook} className="text-xs bg-primary font-bold">
              حفظ وتفعيل Webhook
            </Button>
          </DialogFooter>
        </DialogContent>
