...
export const AppHeader: React.FC<AppHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b bg-card/80 px-4 md:px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-2 w-56 sm:w-72 md:w-96">
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className="flex items-center gap-2 md:gap-3">
        <Badge
          ...
          className="hidden md:inline-flex h-7 gap-1.5 rounded-full px-2.5 text-[10px]"
          title={dataError || undefined}
        >
          <Database className={`h-3 w-3 ${isDataLoading ? "animate-pulse" : ""}`} />
          {dataError ? "خطأ في الاتصال" : dataMode === "live" ? "بيانات مباشرة" : "نسخة تجريبية"}
        </Badge>
        ...
        {isDemo ? (
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-9 gap-1.5 border-primary/30 bg-primary/5 text-xs font-bold text-primary"
          >
            <Shield className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">{roleLabels[currentRole][language]}</span>
          </Button>
